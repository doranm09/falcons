...
Examples:
    python3 modbus_inject.py          # Write 0 (triggers anomaly)
    python3 modbus_inject.py 4500     # Write 4500 (normal range)
    python3 modbus_inject.py 5000     # Write 5000 (high range)
"""
...
def write_average_pressure(
    ...
    try:
        ...
        # Read back to verify
        print(f"[*] Reading back register to verify...")
        ...
        if actual_value == value:
            ...
            print(f"[*] Write succeeded even though the PLC updated the value before verification")
...
Anomaly trigger: Values outside normal range will be detected by IDS process service
    ...
    if not (0 <= args.value <= 65535):
        print(f"[!] Error: Value must be between 0 and 65535 (16-bit register)")
        sys.exit(1)
    if args.duration <= 0:
