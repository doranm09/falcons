...
Architecture baseline: `docs/security_onion_soc_architecture.md`
...
## Search & Explorer
- `GET /dashboard/siem/events/` (via `dashboard/siem_query`) exposes time-range filtering, multi-value filters (`event_type_in`, `source_in`, `event_dataset_in`, `source_ip_in`, `destination_port_in`), pagination, and aggregation buckets. The explorer UI (`dashboard/siem_events.html`) renders search controls, timeline, top sources/types, network endpoints, and OT workflow presets for quick triage.
...
- OpenSearch forwarding is optional; the client (`dashboard/opensearch_client.py`) builds NDJSON bulk payloads with index names like `siem-events-YYYY.MM.DD` and honors TLS/auth. The OpenSearch document keeps the producer as `event_source` and uses ECS-style `source.*` / `destination.*` objects for endpoint fields so schema growth does not collide with the legacy scalar `source` name.
